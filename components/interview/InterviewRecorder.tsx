...
const InterviewRecorder = ({
  ...
  // --- NEW: State for video monitoring ---
  ...
  // --- NEW: Initialize video monitoring ---
  useEffect(() => {
    console.log("Video monitoring system initialized.");
  ...
  useEffect(() => {
    return () => {
      ...
      // --- NEW: Cleanup monitoring interval ---
  ...
  // --- NEW: Function to monitor video activity ---
  const monitorVideoActivity = () => {
    if (videoRef.current && canvasRef.current && isRecording) {
      try {
        ...
        if (!ctx || video.videoWidth === 0 || video.videoHeight === 0) {
          setVideoWarning("Video stream not available. Please check your camera.");
          return;
        }
        ...
        const currentFrameData = imageData.data.toString();
        
        // Simple activity detection - check if frame has changed
        if (lastFrameDataRef.current === null) {
          lastFrameDataRef.current = currentFrameData;
          setVideoWarning(null);
        } else if (lastFrameDataRef.current === currentFrameData) {
          setVideoWarning("Video appears frozen. Please check your camera connection.");
        } else {
          lastFrameDataRef.current = currentFrameData;
          setVideoWarning(null);
        }
        ...
        setVideoWarning("Video monitoring unavailable.");
      }
    }
  };

  // --- NEW: Function to start video monitoring ---
  const startVideoMonitoring = () => {
    if (monitoringIntervalRef.current) {
      clearInterval(monitoringIntervalRef.current);
    }
    monitoringIntervalRef.current = setInterval(monitorVideoActivity, 3000); // Check every 3 seconds
    console.log("Video monitoring started successfully.");
  };

  // --- NEW: Function to stop video monitoring ---
  ...
  return (
    <div className="w-full">
      <div className="flex flex-col items-center gap-4 mb-4">
        ...
        {/* --- NEW: Video Warning Message --- */}
        {videoWarning && !error && ( // Only show if no *critical* error
          <div className="p-3 text-sm text-yellow-700 bg-yellow-100 rounded-md dark:bg-yellow-900/30 dark:text-yellow-400">
            {videoWarning}
          </div>
        )}
        {/* --- End of new warning --- */}
